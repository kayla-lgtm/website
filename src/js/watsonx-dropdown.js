(function () {
    let accordion = document.getElementById("watsonxAccordion");
    let image = document.getElementById("watsonxOfferImage");
    let imageFrame = document.querySelector(".watsonx-image-frame");
    let offersRight = document.querySelector(".watsonx-productivity-right");

    if (!accordion || !image || !imageFrame || !offersRight) {
        return;
    }

    let triggers = accordion.querySelectorAll(".watsonx-offer-trigger");
    let items = accordion.querySelectorAll(".watsonx-offer-item");
    let mobileBreakpoint = 900;

    function getActiveItem() {
        return accordion.querySelector(".watsonx-offer-item.is-active");
    }
    ...
    function activateItem(trigger) {
        let parentItem = trigger.closest(".watsonx-offer-item");
